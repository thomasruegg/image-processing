...
M = 4 # Length of impulse response as well as signal
N = 4
...
gcc = np.zeros(M+N-1, dtype="int") # Initialize result vector of convolution
for n in range(0, M+N-1):  # n = [0, 1, 2, ..., 2*M-2]
    for m in range(0, M):  # m = [0, 1, 2, ..., M-1]
        if -N <= n-m and n-m < N:
